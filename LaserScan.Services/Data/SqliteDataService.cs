...
namespace Kogerent.Services.Implementation
{
    public class SqliteDataService : IDataService
    {
        ...
        public SqliteDataService(ILogger logger)
        {
            Logger = logger;
        }
        ...
        {
            return _connectionString ?? (_connectionString = ConfigurationManager.ConnectionStrings[key].ConnectionString);
        }
        ...
        public (DateTime, List<IntXFloatYPoint>) LoadWidthData(DateTime date, string name = "PirPoints")
        {
            try
            {
                ...
                using (IDbConnection cnn = new SQLiteConnection(connectionString))
                {
                    ...
                    {
                        string sql = $"SELECT * FROM PointsCollection{index} WHERE [Date] >= '{date:yyyy-MM-dd HH:mm:ss.fff}' LIMIT 1";
        ...
        public (DateTime, List<IntXFloatYPoint>) LoadNextWidthData(DateTime date, bool next, string name = "PirPoints")
        {
            try
            {
                ...
                using (IDbConnection cnn = new SQLiteConnection(connectionString))
                {
                    ...
                    {
                        sql = next
                            ? @$"select * from dbo.PointsCollection{index}
                                 where [Date] = ( select Min([Date]) from dbo.PointsCollection{index} 
                                                  where [Date] > '{date:yyyy-MM-dd HH:mm:ss.fff}' ) LIMIT 1"
                            : @$"select * from dbo.PointsCollection{index}
